...
> 🔑 VM=정의·VMI=실행 / 부팅소스 4종 / cloud-init(키+서비스) / 증설=InstanceType
...
| 4.9 LoadBalancer | **고정 IP**, 온프렘=**MetalLB**(L2-ARP기본/L3-BGP), CR 3종(MetalLB/IPAddressPool/L2Advertisement) |
...
> 🔑 웹=Route/비웹=LB / virtctl expose vm / 라벨전파+재시작 / UDN=격리
...
| 5.3 디스크 관리 | 인터페이스: **SCSI=핫플러그**(`sdX`)/VirtIO(`vdX`,중지필요) / **persistent**(Make persistent+재부팅) / resize=**확장만**(`xfs_growfs`) |
...
> 🔑 **URL=import / gz=upload** / 봉인·export=CLI필수 / 복원·봉인=중지
...
> **한 줄 결론**: 스토리지 **URL=import/gz=upload** + 네트워킹 **웹=Route/비웹=LB** + **봉인·export는 CLI** + **SNO 재부팅** — 이 4개가 합격의 척추. 🔥
